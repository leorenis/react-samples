...
const Transacoes = () => {
  ...
  const { transacoes, salvarTransacoes } = useAppContext()

  const handleChange = (field: keyof typeof novaTransacao, value: string | number) => {
    setNovaTransacao((prev) => ({...prev, [field]: value }))
  }

  const handleCriarTransacao = async () => {
    try {
      await salvarTransacoes(novaTransacao)
      setNovaTransacao({ nome: "", valor: 0, tipo: "receita", categoria: "", data: "" })
    } catch (err: unknown) {
